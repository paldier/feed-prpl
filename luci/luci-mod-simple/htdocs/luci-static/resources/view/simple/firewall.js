...
var cbiPolicySelectorValue = form.ListValue.extend({
	__init__: function(/* ... */) {
		...
		this.value('maximum', E('div', { 'style': 'display:inline-block;vertical-align:top;margin-bottom:1em' }, [
		...
		this.value('typical', E('div', { 'style': 'display:inline-block;vertical-align:top;margin-bottom:1em' }, [
		...
		this.value('minimum', E('div', { 'style': 'display:inline-block;vertical-align:top;margin-bottom:1em' }, [
...
var cbiPortForwardSection = form.GridSection.extend({
	...
	renderRowActions: function(section_id) {
		...
		return E('div', { 'class': 'td nowrap cbi-section-actions' }, node);
	},
	...
	return new Promise(function(resolveFn, rejectFn) {
		var dlg = ui.showModal(null, [
			...
			E('div', { 'class': 'right' }, [
				...
				E('button', {
					'class': dangerous ? 'cbi-button-negative' : 'important',
					'click': function() {
						ui.hideModal();
						dlg.style.maxWidth = '';
						resolveFn();
...
function getRuleCounters(section_id) {
	return fs.exec_direct('/usr/sbin/iptables', [ '-t', 'nat', '-n', '-x', '-v', '-L', 'zone_wan_prerouting' ]).then(function(text) {
